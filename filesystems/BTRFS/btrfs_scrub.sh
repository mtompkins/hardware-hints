...
IS_SRB_ALL=$($CHK_SRB)
IS_SRB_RUN=`grep -o 'not running\|finished\|aborted\|no stats available' <<<$IS_SRB_ALL`

if [ "$IS_SRB_RUN" == "no stats available" -o "not running" -o "$IS_SRB_RUN" == "finished" -o "$IS_SRB_RUN" == "aborted" ] ; then
